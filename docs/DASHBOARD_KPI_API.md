...
| **Gastos** | `GET /api/dashboard/expenses/summary`, `.../by-category`, `.../timeseries` (`from`/`to`, `branchId?`, `categoryId?`, `expenseId?`, `granularity?`). Además `.../top-lines` con `categoryId` obligatorio, `expenseId?`, `limit?` (1–500, defecto 15). | `dashboardApi` + `useDashboardGastosSection` + `DashboardGastosSection` (sin recarga completa al cambiar filtros de línea ni cantidad de líneas top). |
