...
| **Gastos** | `GET /api/dashboard/expenses/summary`, `.../by-category`, `.../timeseries` (`from`/`to`, `branchId?`, `categoryId?`, `expenseId?`, `granularity?`). Además `.../top-lines` con `categoryId` obligatorio: devuelve **top N ítems de catálogo por suma** de líneas (agrupa varias compras del mismo ítem), `expenseId?`, `limit?` (1–500, defecto 15). | `dashboardApi` + `useDashboardGastosSection` + `DashboardGastosSection` (tabla + barras comparativas, sin recarga completa). |
| **Domicilios** | **`GET /api/dashboard/delivery`** — `from`, `to`, `branchId?`, `deliveryManId?` (filtra pedidos y ventas por repartidor). **`GET /api/dashboard/delivery/me`** — mismo contrato para el domiciliario autenticado (`fetchDeliveryDashboardForSelf`). | `useDashboardDomiciliosSection` comparte `deliveryEvolutionDriverId` con `useDashboardShellMockState` para refetch. UI: `DeliveryPerformancePanel` (reutilizado en **`/delivery`** pestaña Rendimiento + `DashboardPeriodPanel`). Mock: `VITE_DASHBOARD_DELIVERY_MOCK=true`. |
