...
| **Domicilios** | **`GET /api/dashboard/delivery`** — `from` y `to` obligatorios (UTC ISO), `branchId?`; tiempos medios, repartidores, evolución por bucket. | `fetchDeliveryDashboard` + `useDashboardDomiciliosSection` (**mismo `dateRange` que el sidebar** y `useDashboardShellMockState`). Mock: `VITE_DASHBOARD_DELIVERY_MOCK=true` (sin llamada al API). |
