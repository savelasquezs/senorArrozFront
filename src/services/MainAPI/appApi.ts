...
class AppApi extends BaseApi {
    ...
    async getApps(
        filters?: AppFilters
    ): Promise<ApiResponse<PagedResult<App>>> {
        ...
        return this.get<ApiResponse<PagedResult<App>>>('/apps', {
            params,
        });
    }
    ...
    async getAppById(id: number): Promise<ApiResponse<App>> {
        return this.get<ApiResponse<App>>(`/apps/${id}`);
    }

    // 3. Obtener apps por banco
    async getAppsByBank(bankId: number): Promise<ApiResponse<App[]>> {
        return this.get<ApiResponse<App[]>>(`/apps/by-bank/${bankId}`);
    }
    ...
    async createApp(
        ...
        return this.post<ApiResponse<App>>('/apps', payload);
    }
    ...
    async updateApp(
        ...
        return this.put<ApiResponse<App>>(`/apps/${id}`, payload);
    }

    // 6. Eliminar app
    async deleteApp(id: number): Promise<ApiResponse<string>> {
        return this.delete<ApiResponse<string>>(`/apps/${id}`);
    }
}
