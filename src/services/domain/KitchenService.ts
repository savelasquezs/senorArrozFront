...
export class KitchenService {
	 ...
	 */
	static isReservationTakenPendingKitchenTime(
		order: OrderListItem,
		now: Date = new Date(),
	): boolean {
	 ...
	 */
	static isVisibleToActiveKitchenForAlerts(
		order: OrderListItem,
		now: Date = new Date(),
	): boolean {
		if (order.status !== 'taken' && order.status !== 'in_preparation')
			return false;
	...
	static generateOrderSpeechText(
		order: OrderListItem,
		...
		const spokenNotes = this.formatOrderNotesForVoice(orderNotes);
		const notesSegment = spokenNotes ? `. ${spokenNotes} por favor` : '';
		return `Nuevo pedido,  ${items.join('')}${notesSegment}`;
	}

	static generateOrderNotificationText(
		order: OrderListItem,
		products: Array<{ name: string; quantity: number }>,
	): string {
	...
	private static formatOrderNotesForVoice(
	...
	static buildOrderModifiedNotificationBody(
		...
		if (changes) {
			...
			for (const r of rep) {
				parts.push(
					`Cambio de ${r.previousProductName} por ${r.newProductName}.`,
				);
			}
			const added = changes.addedLines ?? [];
			if (added.length === 1) {
				parts.push(
					`Se agregó ${this.qtyWord(added[0].quantity)} ${added[0].productName}.`,
				);
			...
			if (rem.length === 1) {
				...
				parts.push(
					`Ya el pedido no lleva: ${rem.map((r) => r.productName).join(', ')}.`,
				);
			}
		...
		const mid = usedFallback
			? 'Cambio en el pedido. Verifica en pantalla.'
			: parts.join(' ');
		const closing = usedFallback ? '' : ' Verifica en tu pantalla los cambios.';
		return {
			body: `${intro} ${mid}${closing}`.replace(/\s+/g, ' ').trim(),
			usedFallback,
		};
	}
	...
	static buildOrderModifiedSpeechText(
		...
		const { body } = this.buildOrderModifiedNotificationBody(
			orderId,
			kind,
			changes,
			orderNotes,
