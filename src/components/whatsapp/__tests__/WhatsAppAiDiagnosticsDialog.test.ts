...
describe('WhatsAppAiDiagnosticsDialog usage',()=>{
  it('loads initially with local date-only filters and renders daily billable output',async()=>{ getAiUsage.mockResolvedValueOnce({data:usage()}); const w=mountDialog(); await flushPromises(); expect(getAiUsage).toHaveBeenCalled(); const filters=getAiUsage.mock.calls.at(-1)?.[0]; expect(filters.fromDate).toMatch(/^\d{4}-\d{2}-\d{2}$/); expect(filters.toDate).toMatch(/^\d{4}-\d{2}-\d{2}$/); expect(w.text()).toContain('Uso diario'); expect(w.text()).toContain('Salida facturable'); expect(w.text()).toContain('40'); })
