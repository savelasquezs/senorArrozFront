...
const usageCards = computed(() => usage.value ? [
  ...
  { label: 'Mensajes de contexto', value: (usage.value.averageContextMessages??0).toFixed(1) }, { label: 'Definiciones de herramientas', value: (usage.value.averageToolDefinitions??0).toFixed(1) }, { label: 'Fallback del planificador', value: `${((usage.value.contextPlannerFallbackRate??0)*100).toFixed(1)}%` },
