...
export const useBranchesStore = defineStore('branches', () => {
    ...
    const fetchAll = async (filters?: Partial<BranchFilters>) => {
        try {
            ...
            error.value = null
            const res = await mainApi.getAllBranches(filters)
            list.value = res.data
        } catch (err: any) {
    ...
    const fetch = async (filters?: BranchFilters) => {
        try {
            isLoading.value = true
            error.value = null
            const res = await mainApi.getBranches(filters)
            list.value = res.data
        } catch (err: any) {
            error.value = err.message || 'Error al cargar sucursales'
            throw err
    ...
    const fetchById = async (id: number) => {
        try {
            isLoading.value = true
            error.value = null
            const res = await mainApi.getBranchById(id)
    ...
    const create = async (payload: Pick<Branch, 'name' | 'address' | 'phone1' | 'phone2'>) => {
        try {
            isLoading.value = true
            error.value = null
            const res = await mainApi.createBranch(payload)
            return res.data
        } catch (err: any) {
    ...
    const update = async (id: number, payload: Pick<Branch, 'name' | 'address' | 'phone1' | 'phone2'>) => {
        try {
            isLoading.value = true
            error.value = null
            const res = await mainApi.updateBranch(id, payload)
            current.value = res.data
            return res.data
        } catch (err: any) {
    ...
    return { list, current, isLoading, error, fetchAll, fetch, fetchById, create, update, remove, clear }
})
